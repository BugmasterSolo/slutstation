...
g fish cast - Casts the fishing line at a chosen location.
    ...
    async def cast(self, host, state, args):
        ...
        for i in range(len(self.LOCATIONS)):
            descrip += f"\n{chr(i + 0x41)}. {self.LOC_PRINT[i]} | {self.LOC_PRICES[i]}{host.CURRENCY_SYMBOL}"
        descrip += "```"
        ...
        async with host.db.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                price = (target[8] * self.LOC_PRICES[locindex] / 10)
    ...
    async def trap(self, host, state, args):
        '''Traps allow the user to catch multiple fish over a given duration of time, at a higher cost.
        
You must schedule your traps responsibly -- after a duration of time, they are up for grabs by the rest of the server. However, the longer they are open, the more fish you can catch.
        '''
